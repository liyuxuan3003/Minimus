...
- **常用宏包**：集成常用宏包，例如`physics`、`cleverref`、`graphicx`、`tabularray`等，确保实用命令和环境总是开箱可用。
...
- 引入`cleverref`宏包，解决了引用只有计数而无类型的问题（希望得到图2.1而不是2.1）。
...
- 定义了一系列`\Sum[a][b]`、`\Int[a][b]`、`\Lim[a][b]`等包装命令，避巨算符`\sum_{a}^{b}`繁琐的上下标语法。
...
- `\code`命令提供便捷的行内代码体验，自动处理与相邻中文的间距问题。
